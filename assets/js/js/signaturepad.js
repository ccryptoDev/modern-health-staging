...
// if( acceptSignature ) {
//     alert("acceptSignature");
// 	acceptSignature.addEventListener( 'click', function( event ) {
...
saveButton.addEventListener('click', function(event) {
	...
	var hiddensignatureid = $('#hiddensignatureid').val();
	 ...
	 if(signaturePad.isEmpty() && checkCanvasEmpty != 1){
		...
		if(checkCanvasEmpty == 0){
						$.ajax({
						  type: "POST",
						  url: "/saveSignature?initials="+initial+"&fullname="+fullname+"&hiddensignatureid="+hiddensignatureid,
						  data: {
							 imgBase64: data,
						  },
						}).done(function(o) {
		...
		return false;
		if(!confirm("Signature will be saved"))
			return false;
